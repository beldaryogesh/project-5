...
const createOrder = async function (req, res) {
    try {
        ...
        if (!existUser) {
            return res.status(400).status({ status: false, message: "User have not exist in database." })
        }
        let existCart = await cartModel.findOne({ _id: cartId , userId: userId })
        if (!existCart) {
            return res.status(400).status({ status: false, message: "user doesnt have there specific cart ." })
        }
        ...
        if(cancellable){
            ...
            var cancel = cancellable.toString().trim()
            if(!cancelRegex.test(cancel)){
        ...
        let oderCreate = await orderModel.create(orderDetails)
